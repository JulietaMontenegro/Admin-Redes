...
@Component({
  selector: 'app-delete-materia-alumno',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
  ...
  templateUrl: './delete-materia-alumno.component.html',
  styleUrl: './delete-materia-alumno.component.css'
})
export class DeleteMateriaAlumnoComponent {
  ...
  submitApplication() {
    ...
    const idAlumno = parseInt(this.route.snapshot.params['id'], 10);
    if (this.alumno) {
      this.alumno.materias = this.alumno.materias.filter(numero => numero !== idMateria);
    }
